...
class FileTransferClientTCP extends JFrame implements ActionListener, MouseListener {
    JPanel panel;
    ...
    int len; // track number of files on the server

    public FileTransferClientTCP(String dir, String host, int port) {
        super("TCP CLIENT "/*+IDCount.id*/);
        ...
        try {
            ...
            // sort the array of strings -> displayed in the scrollpane
        ...
        if (event.getSource() == up) {
            try {
                ...
                if (fileExists) {
                    ...
                    // sort the array of strings -> displayed in the scrollpane
                    Arrays.sort(temp_names);
                    // update the contents of the list -> displayed in the scrollpane
            ...
            try {
                ...
                pw.println(file); //tells server which file is to be downloaded
                ...
                if(s.equals("Success")) {
                    ...
                    System.out.println("Requested file not found in the server.");
                    error.setText("Requested file not found in the server.");
                    panel.revalidate();
                }
            }
            catch (Exception exc) {
    ...
    public static void main(String args[]) {
        // need at least three arguments: first one set as directory path,
        // second set as host address ,and third set as port number
        // If host address is not present, set it to "localhost"
        // If port number is not present, default set to 4545
        // If directory path is not present, throw error
        if(args.length >= 3){
            FileTransferClientTCP tcp = new FileTransferClientTCP(args[0], args[1], Integer.parseInt(args[2]));
            tcp.setSize(1000, 900);
            tcp.setVisible(true);
        }
        else if(args.length == 2){
            FileTransferClientTCP tcp = new FileTransferClientTCP(args[0], args[1], 4545);
            tcp.setSize(1000, 900);
            tcp.setVisible(true);
        }
        else if(args.length == 1){
            FileTransferClientTCP tcp = new FileTransferClientTCP(args[0], "localhost", 4545);
            tcp.setSize(1000, 900);
            tcp.setVisible(true);
        }
        else {
